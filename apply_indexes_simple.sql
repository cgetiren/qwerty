...
-- Run this in Supabase SQL Editor (WITHOUT CONCURRENTLY)

-- 1. Index for chats by agent + date + brand
CREATE INDEX IF NOT EXISTS idx_chats_agent_created_brand
...
CREATE INDEX IF NOT EXISTS idx_personnel_brand_name
  ON personnel(brand_id, name)
  WHERE name != 'Unknown';

-- 6. Alerts by brand + date
CREATE INDEX IF NOT EXISTS idx_alerts_brand_created
...
WHERE indexname LIKE 'idx_%'
  ...
  AND tablename IN ('chats', 'chat_analysis', 'personnel_daily_stats', 'personnel', 'alerts')
