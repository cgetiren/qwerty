...
function App() {
  ...
  useEffect(() => {
    if (!session || !currentNav) return;
    logPageView(currentNav.name);
  ...
  return (
    <NotificationProvider>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
        <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white/90 dark:bg-slate-950/90 border-b border-slate-200 dark:border-white/5 px-4 py-3 flex items-center justify-between backdrop-blur-xl">
          <button
            onClick={() => setSidebarOpen(true)}
            className="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white transition-all duration-200"
          >
          ...
          <main className="flex-1 lg:ml-64 pt-20 lg:pt-8 pb-8 px-4 sm:px-6 lg:px-8 min-w-0">
            <Suspense fallback={<PageLoader />}>
              <Routes>
                ...
                <Route path="/personnel-reports" element={<PersonnelReportPage />} />
                <Route path="*" element={<Navigate to="/dashboard" replace />} />
              </Routes>
            </Suspense>
          </main>
