...
export function Tooltip({ content, children, position = 'top' }: TooltipProps) {
  ...
  return (
    ...
    >
      {children}
      {isVisible && content && (
        <div className={`absolute z-[9998] ${positionClasses[position]} animate-fade-in`}>
          <div className="bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white text-sm px-3 py-2 rounded-lg shadow-xl whitespace-nowrap max-w-xs">
            {content}
