...
export default function ChatAnalysisList() {
  ...
  const submitFlag = async () => {
    ...
    try {
      ...
      await supabase.from('objection_logs').insert({
      ...
      logAudit({
        ...
        entityLabel: `${selectedChat.agent_name} - ${selectedChat.customer_name}`,
        description: `Chat analizi isaretlendi: ${flagReason}`,
        oldValues: { score: selectedChat.analysis.overall_score },
  ...
  const resolveFlag = async () => {
    ...
    try {
      ...
      logAudit({
        ...
        entityId: selectedChat.id,
        entityLabel: `${selectedChat.agent_name} - ${selectedChat.customer_name}`,
        description: `Chat analiz itirazi cozumlendi: ${flagResolutionNote}`,
        brandId: activeBrand?.brand_id,
      });
  ...
  const reanalyzeSingleChat = async () => {
    ...
    logAudit({
      ...
      entityId: chatId,
      entityLabel: `${selectedChat.agent_name} - ${selectedChat.customer_name}`,
      description: `Chat yeniden analiz edildi${objectionReason ? ` (itiraz: ${objectionReason})` : ''}`,
      oldValues: selectedChat.analysis ? { score: selectedChat.analysis.overall_score, sentiment: selectedChat.analysis.sentiment } : undefined,
      brandId: activeBrand?.brand_id,
    });
