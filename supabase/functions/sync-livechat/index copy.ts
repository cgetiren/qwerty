...
function processPageChats(pageChats: any[], brandId: string | null, existingAnalyzedSet: Set<string>, apiSendsUtc: boolean = true) {
  ...
  for (const chat of pageChats) {
    ...
    chatRecords.push({
      ...
      rating_score: rawChatData.rating_score || null,
      rating_status: rawChatData.rating_status || (rawChatData.rating_score ? 'rated' : 'not_rated'),
      rating_comment: rawChatData.rating_comment || null,
      has_rating_comment: rawChatData.has_rating_comment || false,
...
async function runSyncWork(supabase: any, jobId: string, startDate: string, endDate: string, brandId: string | null, pageLimit: number = MAX_PAGES_PER_RUN): Promise<object> {
  ...
  while (hasMorePages) {
    ...
    for (let i = 0; i < pageThreadIds.length; i += 500) {
      ...
      const { data: existingChats } = await supabase
        ...
        .eq("analyzed", true);
      if (existingChats) {
        existingChats.forEach((c: any) => existingAnalyzedSet.add(c.id));
      }
    }
    ...
    console.log(`  ✓ ${chatRecords.length} chats upserted`);
  ...
  console.log(`\n✅ Sync complete in ${result.execution_time_seconds}s: ${result.synced} chats, ${result.messages_synced} messages${pageLimitReached ? ' (page limit reached, more data pending)' : ''}`);
  ...
  if (brandId) {
    ...
    // API gecikmeli chat'leri yakalamak icin last_sync_at her zaman en az 60dk geride kalir
    const sixtyMinBack = new Date(now.getTime() - 60 * 60 * 1000).toISOString();
    if (pageLimitReached && newestChatDate) {
      ...
      // Normal sync - 60dk overlap birak
      nextSyncAt = sixtyMinBack;
    }
