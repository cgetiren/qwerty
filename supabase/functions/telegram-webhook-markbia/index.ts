...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: settings } = await supabase
      ...
      .eq("brand_id", MARKBIA_BRAND_ID)
      .maybeSingle();

    if (!settings?.telegram_bot_token) {
      throw new Error("MarkBia Telegram not configured");
    }
    ...
    if (!settings.telegram_chat_id) {
      ...
      console.log(`Auto-configured MarkBia telegram_chat_id: ${incomingChatId}`);
    }
    ...
    console.log("MarkBia webhook received:", JSON.stringify({ rawText, cleanedWithoutSlash, chatType: message.chat.type }));
    ...
    if (cleanedWithoutSlash === "start" || cleanedWithoutSlash === "help") {
      ...
      await sendTelegramMessage(
        settings.telegram_bot_token,
        incomingChatId,
        `<b>MarkBia LiveChat Analiz Botu</b>\n\n` +
    ...
    if (egitimCmdMatch) {
      ...
      if (!chatInfo || chatInfo.brand_id !== MARKBIA_BRAND_ID) {
        await sendTelegramMessage(
          settings.telegram_bot_token,
          incomingChatId,
    ...
    console.error("MarkBia Telegram webhook error:", error);
    return new Response(
