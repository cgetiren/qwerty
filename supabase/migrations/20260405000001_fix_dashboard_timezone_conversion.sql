-- Fix: Dashboard timezone conversion bug
-- Problem: ::timestamptz AT TIME ZONE 'Europe/Istanbul' was shifting the window by +6 hours
-- because timestamptz->AT TIME ZONE returns timestamp, which is then re-interpreted as UTC
-- Solution: Use ::timestamp AT TIME ZONE which correctly interprets the input AS Istanbul time
-- This makes Dashboard show the same date range as ChatList (midnight-to-midnight Istanbul)
...
BEGIN
  IF p_start_date IS NOT NULL AND p_end_date IS NOT NULL THEN
    -- ::timestamp (without tz) + AT TIME ZONE interprets the value AS Istanbul time
    -- '2026-04-05T00:00:00'::timestamp AT TIME ZONE 'Europe/Istanbul' = 2026-04-04T21:00:00Z
